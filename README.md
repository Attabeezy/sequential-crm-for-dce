...
Preliminary evaluation on this benchmark compares six models under 5-fold CV. Key finding: well-engineered static features recover most default-predictive signal (LogisticRegression leads at 0.914 AUC-ROC, ahead of XGBoost 0.881, RandomForest 0.884, and LightGBM 0.878); standalone LSTM collapses (0.587); Hybrid LSTM (0.862) improves on standalone LSTM but still trails the best static models. Results are scoped to the synthetic benchmark — real-data validation (Paper B, Telecel Ghana) is ongoing.
