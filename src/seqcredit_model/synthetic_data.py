"""Synthetic mobile money transaction data generator with credit behavior."""
...
class CalibratedMoMoDataGenerator:
    """Generate realistic mobile money transactions with loan activity."""
    ...
    def __init__(
        ...
        print(f"Initialized for {n_users:,} users, output: {self.output_dir}")

    def _create_recipient_pool(self):
        """Pre-generate 1,000 reusable recipient profiles to simulate recurring payees."""
        self.recipient_pool = [
            {
    ...
    def _assign_credit_archetype(self):
        """Sample a credit archetype according to the class-level weight distribution."""
    ...
    def generate_user_profile(self, user_id):
        """Generate a user profile with credit behavior attributes."""
        ...
        profile = {
            ...
            "amount_mu": np.random.normal(self.calibration["amount_lognormal_mu"], 0.2),
            "amount_sigma": max(
                0.5, np.random.normal(self.calibration["amount_lognormal_sigma"], 0.1)
            ),
            "hours_between_txns": max(
                2,
                np.random.gamma(
                    shape=2,
    ...
    def _generate_loan_profile(self, archetype_config):
        """Return loan behaviour parameters for a borrowing user."""
        # Number of loans the user will attempt during the simulation window
        loan_frequency_map = {
            "regular": np.random.randint(2, 5),
            "occasional": np.random.randint(1, 3),
            "frequent": np.random.randint(4, 7),
            "aggressive": np.random.randint(3, 6),
        }

        # Tuple: (p_on_time, p_late, p_very_late) — probabilities must sum to 1.0
        repayment_timing_map = {
            "on_time": (0.9, 0.05, 0.05),
        ...
        return {
            ...
            "repayment_timing": repayment_timing_map.get(
                repay_behavior, (0.6, 0.3, 0.1)
            ),
            ...
            "loan_amount_preference": np.random.uniform(0.5, 0.9),
        }

    def generate_transaction_amount(self, user_profile, txn_type):
        """Generate transaction amount based on type and user preferences."""
        base_amount = np.random.lognormal(
            user_profile["amount_mu"], user_profile["amount_sigma"]
        )
    ...
    def calculate_fees(self, amount, txn_type, user_accepts_fees):
        """Calculate transaction fees and e-levy."""
        fees = 0.0
        elevy = 0.0
    ...
    def _select_transaction_type(self, user_profile, current_balance):
        """Select transaction type based on user profile and balance."""
    ...
    def _generate_timestamp(self, current_date, user_profile):
        ...
        if np.random.random() < user_profile["pref_night"]:
            ...
            hour = int(np.clip(np.random.normal(user_profile["pref_hour"], 3), 6, 22))
    ...
    def _select_recipient(self, txn_type, user_recipients):
        ...
        if txn_type in ["TRANSFER", "PAYMENT_SEND", "CASH_OUT"]:
            ...
            return {"phone": "0", "name": provider, "account": f"{provider}_ACCT"}

    def generate_loan_transaction(
        self,
        user_profile,
        current_date,
        current_balance,
        ...
        """Generate CREDIT or LOAN_REPAYMENT transaction."""
        provider = user_profile.get("preferred_loan_provider", "QWIKLOAN")

        if is_disbursement:
            credit_limit = user_profile.get(
                "credit_limit", self.loan_params["initial_limit"]
            )
            ...
            amount = round(np.random.uniform(min_loan, max_loan * pref), 2)
            amount = max(min_loan, min(amount, max_loan))

            transaction = {
                "TRANSACTION DATE": self._generate_timestamp(
                ...
                "LOAN_DUE_DATE": (
                    current_date + timedelta(days=self.loan_params["term_days"])
                ).strftime("%Y-%m-%d"),
            }
            ...
            repay_amount = min(total_due, current_balance - 5)
            ...
            transaction = {
                ...
                "LOAN_PROVIDER": provider,
                "LOAN_PRINCIPAL_PAID": round(
                    repay_amount
                    - (
                        repay_amount
                        * self.loan_params["interest_rate"]
                        / (1 + self.loan_params["interest_rate"])
                    ),
                    2,
                ),
                "LOAN_INTEREST_PAID": round(
                    repay_amount
                    * self.loan_params["interest_rate"]
                    / (1 + self.loan_params["interest_rate"]),
                    2,
    ...
    def _determine_repayment_timing(self, user_profile, loan_date):
        """Determine repayment timing based on user archetype."""
        ...
        timing_choice = np.random.choice(
            ["on_time", "late", "very_late"], p=timing_probs
        )
    ...
    def generate_transactions_for_user(self, user_profile):
        ...
        current_date = self.start_date + timedelta(
            days=user_profile["account_start_day"]
        )
        ...
        loans_taken = 0
        loan_target = (
            user_profile.get("loan_count_target", 0)
            if user_profile["takes_loans"]
            else 0
        )
        credit_risk_labels = []

        # Pre-schedule loan injection points evenly across the transaction sequence
        loan_at_txn_indices = []
        if loan_target > 0:
            total_expected = n_transactions
            first_loan_idx = max(3, int(total_expected * 0.15))
            loan_interval = max(
                3, (total_expected - first_loan_idx) // (loan_target + 1)
            )

            for i in range(loan_target):
                loan_idx = (
                    first_loan_idx + (i * loan_interval) + np.random.randint(-1, 2)
                )
        ...
        # Main simulation loop: advance through time until end_date or txn quota reached
        txn_idx = 0
        regular_txn_count = 0
        while current_date < self.end_date and regular_txn_count < n_transactions:
            txn_idx += 1

            # --- Loan disbursement: inject when a scheduled index is reached and no loan is active
            if (
                loan_at_txn_indices
                and regular_txn_count >= loan_at_txn_indices[0]
                ...
                if loan_txn and loan_amount > 0:
                    ...
                    if repay_timing is None:
                        ...
                        if remaining_txns > 3:
                            ...
                            repay_at_txn = regular_txn_count + max(
                                1, remaining_txns - 1
                            )
                    ...
                    # Grow credit limit on successful on-time repayment
                    if (
                        user_profile.get("credit_limit_growth", False)
                        and risk_label == 0
                    ):
                        current_limit = user_profile.get(
                            "credit_limit", self.loan_params["initial_limit"]
                        )
                        new_limit = min(
            ...
            # --- Loan repayment: trigger once enough transactions have elapsed since disbursement
            ...
            # --- Regular transaction: select type, generate amount, fees, and recipient
            ...
            # Cap withdrawal to avoid breaching the minimum balance threshold
            ...
            # Advance clock by exponential inter-arrival time; compress on active weekends
            hours_gap = np.random.exponential(user_profile["hours_between_txns"])
            if (
                current_date.weekday() >= 5
                and np.random.random() < user_profile["pref_weekend"]
            ):
                hours_gap *= 0.7
        ...
        # Worst-case risk label across all loans determines the user's final credit label
        if credit_risk_labels:
            final_risk_label = max(credit_risk_labels)
        else:
            final_risk_label = -1
    ...
    def save_user_transactions(self, user_id, transactions):
        """Save user transactions to CSV file."""
        if not transactions:
            return None
    ...
    def generate_dataset(self):
        ...
        for i in range(self.n_users):
            if (i + 1) % 1000 == 0:
                print(f"  Progress: {i + 1:,}/{self.n_users:,}")
        ...
        if borrowers > 0:
            ...
            print(
                f"\nDefault rate (borrowers): {defaults:,}/{borrowers:,} ({default_rate:.2f}%)"
            )
