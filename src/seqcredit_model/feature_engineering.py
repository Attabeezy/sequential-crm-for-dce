"""Temporal feature extraction from mobile money transaction data."""
...
class TemporalTransactionFeatureEngineer:
    """Extract features from sequential transaction data."""
    ...
    def extract_all_features(self, df, windows=[3, 7, 14, 30]):
        ...
        # --- Amount transforms (compress right-skewed GHS amounts for linear models)
        df["log_amount"] = np.log1p(df["AMOUNT"])
        df["sqrt_amount"] = np.sqrt(df["AMOUNT"])
        df["amount_squared"] = df["AMOUNT"] ** 2

        # --- Amount size bins (GHS thresholds: micro <10, small 10-50, medium 50-200, large 200+)
        ...
        # --- Fee features (fee presence and relative cost signal transaction formality)
        ...
        # --- Cyclical (sin/cos) encodings — avoids the discontinuity at midnight / end of week
        df["hour_sin"] = np.sin(2 * np.pi * df["hour"] / 24)
        df["hour_cos"] = np.cos(2 * np.pi * df["hour"] / 24)
        df["day_sin"] = np.sin(2 * np.pi * df["day_of_week"] / 7)
        df["day_cos"] = np.cos(2 * np.pi * df["day_of_week"] / 7)

        # --- Balance dynamics (log-scale balance + direction and magnitude of change)
        ...
        df["is_large_relative_to_balance"] = (
            df["amount_to_balance_ratio"] > 0.5
        ).astype(int)
        df["will_deplete_balance"] = (
            (df["BAL AFTER"] < 10) & (df["BAL BEFORE"] > 10)
        ).astype(int)

        # --- Sequence position (where in the history this transaction falls)
        df["time_since_last_txn_hours"] = (
            df["TRANSACTION DATE"].diff().dt.total_seconds() / 3600
        )
        ...
        # --- Time-based rolling windows (3d / 7d / 14d / 30d)
        ...
        # --- Behavioural patterns (recipient diversity and transaction type variety)
        recipient_counts = []
        seen_recipients = set()
    ...
    def create_user_level_summary(self, df):
        """Create aggregate features at user level."""
        df = df.copy()
        df["TRANSACTION DATE"] = pd.to_datetime(df["TRANSACTION DATE"])
        ...
        user_features = {
            ...
            "cv_transaction_amount": df["AMOUNT"].std() / df["AMOUNT"].mean()
            if df["AMOUNT"].mean() > 0
            else 0,
            ...
            "pct_payments": (
                (df["TRANS. TYPE"] == "PAYMENT") | (df["TRANS. TYPE"] == "PAYMENT_SEND")
            ).mean(),
            "avg_hours_between_txns": df["TRANSACTION DATE"]
            .diff()
            .dt.total_seconds()
            .mean()
            / 3600
            ...
            "account_age_days": (
                df["TRANSACTION DATE"].max() - df["TRANSACTION DATE"].min()
            ).days,
            "transactions_per_day": len(df)
            / max(
                (df["TRANSACTION DATE"].max() - df["TRANSACTION DATE"].min()).days, 1
            ),
        }
...
def build_user_feature_dataset(
    transactions_dir=str(TRANSACTIONS_DIR), output_path=str(USER_FEATURES_FILE)
):
    """
    Process all user transaction files and build feature dataset.

    Args:
        transactions_dir: Directory with per-user CSV files
        output_path: Where to save feature dataset
    ...
    print(f"Building features from {total_files} user files...")
    ...
    print(f"Done. {len(user_features_df)} users, {skipped} skipped.")
    print(f"Features: {user_features_df.shape}")
